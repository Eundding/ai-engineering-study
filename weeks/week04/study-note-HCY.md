...
시스템 프롬프트가 성능을 사용자 프롬프트보다 향상 시킬 수 있는 이유는
...
**보안 점검 자동화 도구**에는 Azure/PyRIT, leondz/garak, greshake/llm-security, CHATS=labpersuasive_jailbreaker 등이 있고, 이 도구들은 주로 알려진 공격 패턴을 템플릿으로 가지고  모델을 자동으로 테스트한다. 
...
: 데이터베이스 변경처럼 시스템에 큰 영향을 주는 명령은 명시적 승인을 거치도록 규칙을 둔다.
